...
<%-- 								<th class="id provincia hidden" data-prop="idProvincia"></th> --%>
...
// 			data.push(this.idProvincia);
// 			data.push(this.provincia.nombre);
	...
	function insertOrUpdate(data) {
		...
		$.ajax({
			...
			success: function (data) {
				if (data.Result) {
					result = data.Id || 0;
					MessageBox.showSuccess("Alumno guardado correctamente");
				} else if (data.Info) {
					MessageBox.showInfo(data.Info);
				} else {
					var errorMessage = "Ha ocurrido un error al guardar el alumno, intente nuevamente en unos minutos";
					if (data.Error) {
	...
	function deleteObj(id) {
		...
		$.ajax({
			...
			success: function (data) {
				if (data.Result) {
					result = true;
					MessageBox.showSuccess('Alumno eliminado exitosamente');
				} else {
					var errorMessage = "Ha ocurrido un error al eliminar el alumno, intente nuevamente en unos minutos";
					if (data.Error) {
						errorMessage = data.Error;
					}
					MessageBox.showError(errorMessage);
